...
class QSOCNN(object):
    ...
    def _model_(self,input_shape, hidden_num_units, output_num_units, pool_size, nepochs, batch_size):
        ...
        self.x = MaxPooling2D(pool_size=self.pool_size, strides=(2, 2))(self.x)
        self.x = Convolution2D(25, (5, 5), activation='relu',strides=(1, 1), padding='same')(self.x)
        self.x = Convolution2D(25, (3, 3), activation='relu',strides=(1, 1))(self.x)
        self.x = MaxPooling2D(pool_size=self.pool_size, strides=(2, 2))(self.x)
        self.x = Convolution2D(35, (3, 3), activation='relu',strides=(1, 1), padding='same')(self.x)
        self.x = Convolution2D(35, (3, 3), activation='relu',strides=(1, 1))(self.x)
        ...
        self.model = Model(inputs=self.inputs, outputs=self.outputs, name="QSOCNN2021")
